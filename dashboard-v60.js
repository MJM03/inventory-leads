...
function dashboard(){
 ...
 const follow=document.getElementById('agpFollow');if(follow)follow.onclick=()=>document.querySelector('.moduleBtn[data-module="accepted"]')?.click();
 const backup=document.getElementById('agpBackup');if(backup)backup.onclick=openBackup;
...
function boot(){dashboard();document.addEventListener('change',e=>{if(['out','followFilter','followStageFilter'].includes(e.target.id))setTimeout(dashboard,80)});window.addEventListener('storage',dashboard);window.addEventListener('agp:firebase-sync',()=>setTimeout(dashboard,40));}
