(()=>{
const K=['inventoryLeadOutreach','inventoryLeadMessages','inventoryLeadVariants','inventoryLeadVariantResults','inventoryLeadReplies','inventoryLeadAccepted','agpPrintableQuote'];
...
const statusFor=l=>outreach()[l.id]||'Sin contactar';
function dashboard(){
 ...
 d.innerHTML=`<div class="dashHead"><div><span class="miniLabel">Centro de trabajo</span><h2>Hoy en AGP</h2><p>Lo importante para avanzar ventas, sin perder tiempo buscando.</p></div><button id="agpBackup" class="dashGhost">Respaldo</button></div><div class="dashMetrics"><button data-act="pending"><b>${pending}</b><span>Por contactar</span></button><button data-act="follow"><b>${overdue}</b><span>Seguimientos vencidos</span></button><button data-act="interested"><b>${interested}</b><span>Con respuesta/interés</span></button><button data-act="follow"><b>S/ ${Math.round(quotes)}</b><span>Cotizaciones abiertas</span></button></div><div class="dashActions"><button id="agpNext" class="dashPrimary">Contactar siguiente <span>→</span></button><button id="agpFollow" class="dashSecondary">Ver seguimiento</button></div><div class="dashHint">${pending?`Tienes ${pending} prospectos todavía sin contactar.`:'Ya contactaste toda la base disponible.'}${sent?` ${sent} esperan respuesta.`:''}</div>`;
 ...
 document.getElementById('agpNext').onclick=contactNext;document.getElementById('agpBackup').onclick=openBackup;
}
function contactNext(){
 const l=leads().filter(x=>statusFor(x)==='Sin contactar').sort((a,b)=>(b.closeScore||b.score||0)-(a.closeScore||a.score||0))[0];
 if(!l){alert('No quedan prospectos sin contactar.');return}
 const q=document.getElementById('q');if(q){q.value=l.company;q.dispatchEvent(new Event('input',{bubbles:true}))}
 setTimeout(()=>{const card=document.querySelector('#grid .card');const detail=[...card?.querySelectorAll('button')||[]].find(b=>/detalle|ver/i.test(b.textContent)); if(detail)detail.click();else card?.scrollIntoView({behavior:'smooth',block:'center'})},120);
}
function openBackup(){
 let dlg=document.getElementById('backupDialog');if(!dlg){dlg=document.createElement('dialog');dlg.id='backupDialog';dlg.innerHTML=`<div class="sheet"><div class="sheetTop"><div><h2>Respaldo AGP</h2><div class="meta">Protege el seguimiento guardado en este dispositivo.</div></div><button class="close" id="backupClose">×</button></div><div class="backupActions block"><button class="btn primary wide" id="exportBackup">Exportar respaldo</button><label class="btn wide backupImport">Importar respaldo<input id="importBackup" type="file" accept="application/json" hidden></label><p>El archivo incluye estados, mensajes, respuestas, seguimiento y cotizaciones guardadas localmente.</p></div></div>`;document.body.appendChild(dlg);dlg.querySelector('#backupClose').onclick=()=>dlg.close();dlg.querySelector('#exportBackup').onclick=exportBackup;dlg.querySelector('#importBackup').onchange=importBackup}dlg.showModal();
}
...
function boot(){dashboard();setTimeout(dashboard,300);document.addEventListener('change',e=>{if(['out','followFilter','followStageFilter'].includes(e.target.id))setTimeout(dashboard,100)});window.addEventListener('storage',dashboard)}
