(()=>{
  ...
  // Commercial rule requested for AGP CRM: when a public phone exists but
  // WhatsApp was not separately confirmed, use that same number as the
  // WhatsApp contact for outreach.
  leads.forEach(l=>{
  ...
  function decorate(root=document){
    root.querySelectorAll?.('.contact').forEach(el=>{
      if(el.querySelector('.waInfoIcon')) return;
      ...
      while(node=walker.nextNode()){
        if(node.nodeValue.includes('💬')){
          ...
          if(parts[0]) frag.append(document.createTextNode(parts[0]));
          const wrap=document.createElement('span');
          wrap.innerHTML=svg;
          frag.append(wrap.firstElementChild,document.createTextNode(parts.slice(1).join('💬').replace(/^\s*/, ' ')));
  ...
  css.textContent=`
    .waInfoIcon{display:inline-flex;vertical-align:-3px;width:18px;height:18px;margin-right:4px;color:#25D366;flex:0 0 18px}
    .waInfoIcon svg{width:18px;height:18px;display:block}
  `;
  ...
  function boot(){
    ...
    const obs=new MutationObserver(muts=>muts.forEach(m=>m.addedNodes.forEach(n=>{
      if(n.nodeType===1){decorate(n); if(n.matches?.('.contact')) decorate(n.parentNode||document)}
    })));
