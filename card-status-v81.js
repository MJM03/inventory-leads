...
function paintCard(card){const box=card.querySelector('.score'),id=idFromCard(card);if(!box||!id)return;const [icon,text,cls]=state(id),key=`${cls}|${icon}|${text}`;if(box.dataset.cardState===key)return;box.dataset.cardState=key;box.className='score cardState81 '+cls;box.innerHTML=`<span>${icon}</span><b>${text}</b>`;box.title='Estado comercial actual'}
